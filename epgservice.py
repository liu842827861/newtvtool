...
def updateChannelCCTV(fHandle, channelID, channelName):
    try:
        ...
        #Get data
        request = urllib2.Request("http://tv.cntv.cn/epg" % (channelID, dateInChina.strftime("%Y%m%d")))
...
def updateChannelPHNX(fHandle, channelID, siteID, channelName):
    try:
        ...
        #Get data
        request = urllib2.Request("https://www.tvsou.com/epg/" % (siteID, dateInChina.strftime("%Y%m%d")))
        resp = urllib2.urlopen(request)
        data = resp.read().decode("utf-8")
